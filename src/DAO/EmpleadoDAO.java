...
public class EmpleadoDAO implements CrudPersonaInterface<Empleado> {
    ...
    public List<Empleado> listar(String Texto, int TotalPorpagina, int numPagina) {
        ...
        try {
            ps = CON.conectar().prepareStatement("Select*from Empleado_dat where nombre like ?  ORDER by nombre asc limit ?,?; ");
            ps.setString(1,"%"+Texto+"%");
            ps.setInt(2,(numPagina-1)*TotalPorpagina);
            ps.setInt(3, TotalPorpagina);
            rs = ps.executeQuery();
            while (rs.next()){
                registros.add(new Empleado(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getBoolean(5),rs.getString(6),rs.getString(7),rs.getString(8),rs.getString(9),rs.getString(10),rs.getString(11),rs.getString(12),rs.getString(13),rs.getString(14),rs.getString(15),rs.getInt(16),rs.getString(17),rs.getString(18)));
            }
            ps.close();
            rs.close();
    ...
    public Empleado login(int Id, String Contrasena ){
        Empleado ver= null;
        try {
            ps = CON.conectar().prepareStatement("Select*from Empleado_dat where Empleado_id=? AND contrasena=?");
            ps.setInt(1,Id);
            ps.setString(2,Contrasena);
            rs = ps.executeQuery();
            if(rs.next()){
                ver = new Empleado(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getBoolean(5),rs.getString(6),rs.getString(7),rs.getString(8),rs.getString(9),rs.getString(10),rs.getString(11),rs.getString(12),rs.getString(13),rs.getString(14),rs.getString(15),rs.getInt(16),rs.getString(17),rs.getString(18));
            }
            ps.close();
            rs.close();
        } catch (SQLException e) {
    ...
    public boolean existe(String texto) {
        resp=false;
        try {
            ps = CON.conectar().prepareStatement("SELECT NOMBRE FROM Empleado_dat where NOMBRE=?");
            ps.setString(1,texto);
            rs = ps.executeQuery();
            if(rs.next()){
                resp=true;
